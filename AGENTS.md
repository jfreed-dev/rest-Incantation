...
- HTML templates are in `templates/` (`index.html`, `submit_url.html`, `credentials.html`).
...
- `pip install -r requirements.txt` is referenced in `README.md`, but a `requirements.txt` file is not currently present; add one if you formalize dependencies.
...
- If you add tests, prefer `pytest`, place them under `tests/`, and name files `test_*.py`.
